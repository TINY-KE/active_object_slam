...

{
    ...
    // ORB extraction
    ExtractORB(0,imGray);   //对imGray提取特征点，并存储在mvKeys中
    ...
    mvpMapPoints = vector<MapPoint*>(N,static_cast<MapPoint*>(NULL));   // 根据ExtractORB的结果，初始化长度
    mvbOutlier = vector<bool>(N,false);
...
void Frame::ComputeStereoFromRGBD(const cv::Mat &imDepth)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(d>0)   /* 对于RGBD输入,如果某个特征点的深度值有效,那么这里将会反向计算出假想的"右目图像中对应特征点的坐标". */
        {
            mvDepth[i] = d;
...
void Frame::ComputePlanesFromOrganizedPointCloud(const cv::Mat &imDepth)
{
    ...
    //TODO: 参数传递
    int cloudDis = 3;       // 3
    int min_plane = 1000;    //1000
    float AngTh = 3.0;      //3.0
    float DisTh = 0.05;     //0.05
